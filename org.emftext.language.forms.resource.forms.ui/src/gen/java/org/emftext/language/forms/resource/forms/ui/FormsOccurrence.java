 ...
 */
public class FormsOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.emftext.language.forms.resource.forms.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.emftext.language.forms.resource.forms.ui.occurences.declaration";
	...
	private org.emftext.language.forms.resource.forms.ui.FormsTokenScanner tokenScanner;
	...
	private org.emftext.language.forms.resource.forms.IFormsTextResource textResource;
	 ...
	 */
	public FormsOccurrence(org.emftext.language.forms.resource.forms.IFormsTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.emftext.language.forms.resource.forms.ui.FormsTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.emftext.language.forms.resource.forms.mopp.FormsMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.emftext.language.forms.resource.forms.ui.FormsBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.emftext.language.forms.resource.forms.ui.FormsPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, org.emftext.language.forms.resource.forms.ui.FormsPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.emftext.language.forms.resource.forms.ui.FormsPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.emftext.language.forms.resource.forms.ui.FormsPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.emftext.language.forms.resource.forms.ui.FormsOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.emftext.language.forms.resource.forms.ui.FormsOccurrence.DECLARATION_ANNOTATION_ID);
	}
